...
String scheduleDate(DateTime date) =>
    '${monthNames[date.month - 1].substring(0, 3)} ${date.day}';
...
class ScheduleEntry {
  ...
  String get dateRange {
    final dates = episodes.map((e) => e.airDate).whereType<DateTime>().toList()
      ..sort();
    if (dates.isEmpty) return 'Date TBA';
    final first = dates.first, last = dates.last;
    return day(first) == day(last)
        ? scheduleDate(first)
        : '${scheduleDate(first)} – ${scheduleDate(last)}';
  }
...
/// A release card contains only episodes of one show, season and air date.
List<ScheduleEntry> groupReleasesByDay(Iterable<ScheduleEntry> entries) {
  final groups = <(String, int, DateTime?), Map<String, EpisodeData>>{};
  final titles = <String, TitleData>{};
  for (final entry in entries) {
    titles[entry.title.key] = entry.title;
    for (final episode in entry.episodes) {
      final date = episode.airDate;
      final key = (
        entry.title.key,
        episode.season,
        date == null ? null : day(date),
      );
      groups.putIfAbsent(key, () => {})[episode.key] = episode;
    }
  }
  final result = [
    for (final group in groups.entries)
      ScheduleEntry(
        titles[group.key.$1]!,
        group.key.$2,
        group.value.values.toList()..sort(episodeOrder),
      ),
  ];
  result.sort((a, b) {
    final byDate = (a.episodes.first.airDate ?? DateTime(9999)).compareTo(
      b.episodes.first.airDate ?? DateTime(9999),
    );
    if (byDate != 0) return byDate;
    final byTitle = a.title.title.compareTo(b.title.title);
    return byTitle != 0 ? byTitle : a.season.compareTo(b.season);
  });
  return result;
}
...
ScheduleTimeline buildScheduleTimeline(
  ...
  final historical = [
    for (final bundle in bundles)
      SeriesBundle(bundle.title, [
        for (final season in bundle.seasons)
          SeasonData(bundle.title.id, {
            ...season.raw,
            'episodes': [
              for (final episode in season.episodes)
                if (episode.airDate != null &&
                    day(episode.airDate!).isBefore(week))
                  episode.raw,
            ],
          }),
      ]),
  ...
  final years = {
    for (final bundle in historical)
      for (final episode in bundle.episodes)
        if (!episode.isSpecial) episode.airDate!.year,
  }.toList()..sort();
  final past = [
    for (final year in years) ...buildSchedule(historical, now, year: year),
  ];
  final upcoming = buildSchedule(bundles, week);
  final currentLabel =
      '${scheduleDate(week)} – ${scheduleDate(DateTime(week.year, week.month, week.day + 6))} · ${week.year}';
...
List<ScheduleSection> buildSchedule(
  Iterable<SeriesBundle> series,
  DateTime now, {
  int? year,
}) {
  ...
  for (final bundle in series) {
    for (final episode in bundle.episodes) {
      ...
      if (year != null ? date.year != year : day(date).isBefore(today)) {
        continue;
      }
  ...
  List<ScheduleEntry> entries(Map<String, List<EpisodeData>> groups) {
    final result = [
      for (final group in groups.entries)
        ScheduleEntry(
          titles[group.key]!,
          group.value.first.season,
          group.value..sort(episodeOrder),
        ),
    ];
    result.sort((a, b) {
      DateTime first(ScheduleEntry e) => e.episodes
          .map((e) => e.airDate ?? DateTime(9999))
