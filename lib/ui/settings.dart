...
class _SettingsScreenState extends ConsumerState<SettingsScreen> {
  ...
  Future<void> emailSupport() async {
    ...
    try {
      ...
      if (!opened && mounted) {
        message(context, 'No email app available. Contact $supportEmail.');
      }
    } catch (_) {
      if (mounted) {
        message(context, 'Could not open email. Contact $supportEmail.');
      }
    }
  }
  ...
  Future<void> save({
    ...
    try {
      ...
      if (mounted) {
        setState(
          () => error = 'Alerts could not be updated. Allow notifications in your device settings and try again.',
        );
      }
    } finally {
      if (mounted) {
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          Card(
            ...
            child: ListTile(
              ...
              subtitle: Text(
                entries == null
                    ? 'Loading…'
                    : '${entries.where((e) => e.title.isTv).length} shows · ${entries.where((e) => !e.title.isTv).length} movies',
              ),
            ),
          ),
          ...
          if (issue != null)
            Padding(padding: const EdgeInsets.all(12), child: Text(issue)),
          if (alerts.supportsSystemSettings)
            ListTile(
              contentPadding: const EdgeInsets.symmetric(horizontal: 16),
              title: const Text('iPhone notification settings'),
              subtitle: const Text(
                'Choose banners, sounds and badges in iOS Settings',
              ),
          ...
          if (value != null) ...[
            SwitchListTile(
              ...
              subtitle: Text(
                alerts.supported
                    ? 'Reminders for unwatched episodes in your library'
                    : 'Available on iOS and Android',
              ),
              value: value.enabled,
              onChanged: alerts.supported && !busy
                  ? (v) => save(enabled: v)
                  : null,
            ),
            ...
            child: Column(
              children: [
                ListTile(
                  ...
                  subtitle: const Text(supportContactLabel),
                ...
                ListTile(
                  ...
                  trailing: const Icon(Icons.chevron_right),
                  onTap: () =>
                      openScreen(context, const LegalScreen(privacy: false)),
                ),
                ListTile(
                  title: const Text('Privacy Policy'),
                  trailing: const Icon(Icons.chevron_right),
                  onTap: () =>
                      openScreen(context, const LegalScreen(privacy: true)),
                ),
