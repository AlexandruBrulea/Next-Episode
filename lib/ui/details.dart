...
class TitleHeader extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    ClipRRect(borderRadius: BorderRadius.circular(24), child: SizedBox(height: 320, child: Stack(fit: StackFit.expand, children: [
      Poster(title.backdrop.isNotEmpty ? title.backdrop : title.poster, width: double.infinity, height: 320),
      ...
      Positioned(left: 20, right: 20, bottom: 20, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Text(title.title, style: Theme.of(context).textTheme.headlineLarge?.copyWith(fontWeight: FontWeight.bold)),
      ])),
    ]))),
    const SizedBox(height: 20),
    if (string(title.raw['tagline']).isNotEmpty) Padding(padding: const EdgeInsets.only(bottom: 12),
      child: Text(string(title.raw['tagline']), style: Theme.of(context).textTheme.bodyLarge?.copyWith(fontStyle: FontStyle.italic))),
    Wrap(spacing: 18, runSpacing: 8, children: [Text('★ ${title.ratingLabel}'), Text(dateLabel(title.releaseDate)),
      if (integer(title.raw['vote_count']) > 0) Text('${integer(title.raw['vote_count'])} votes')]),
    const SizedBox(height: 14),
    Wrap(spacing: 8, runSpacing: 8, children: [
      Chip(label: Text(title.statusLabel)),
      if (title.isTv) ...[Chip(label: Text('${title.seasonCount} seasons')), Chip(label: Text('${title.episodeCount} episodes'))],
      for (final genre in title.genres) Chip(label: Text(genre)),
    ]),
    const SizedBox(height: 20),
    Text(title.overview, style: Theme.of(context).textTheme.bodyLarge?.copyWith(height: 1.55)),
    const SizedBox(height: 20),
    LibraryAction(title),
...
class _SeriesScreenState extends ConsumerState<SeriesScreen> {
  ...
  Widget build(BuildContext context) => Scaffold(
    ...
    body: ref.watch(seriesDetailsProvider(widget.id)).when(
      loading: () => const Center(child: CircularProgressIndicator()),
      error: (e, _) => FailureView(e, () => ref.invalidate(seriesDetailsProvider(widget.id))),
      data: (loaded) {
        final snapshot = ref.watch(libraryProvider).asData?.value;
        ...
        final saved = snapshot?.contains(title.key) ?? false;
        final seasons = [...bundle.seasons]..sort((a,b) => a.number.compareTo(b.number));
        final season = seasons.where((s) => s.number == selectedSeason).firstOrNull
            ?? seasons.where((s) => s.number > 0).firstOrNull ?? seasons.firstOrNull;
        final progress = WatchProgress.calculate(bundle, watched, DateTime.now());
        return ListView(primary: true, padding: const EdgeInsets.fromLTRB(20, 8, 20, 40), children: [
          if (loaded.offline) const Text('Offline: showing the last saved version.'),
          TitleHeader(title),
          SegmentedButton<bool>(segments: const [
            ButtonSegment(value: false, label: Text('Episodes'), icon: Icon(Icons.play_circle_outline)),
            ButtonSegment(value: true, label: Text('Details'), icon: Icon(Icons.info_outline)),
          ], selected: {details}, onSelectionChanged: (value) => setState(() => details = value.single)),
          const SizedBox(height: 24),
          if (details) TitleInformation(title) else ...[
            SingleChildScrollView(scrollDirection: Axis.horizontal, child: Row(children: [
              for (final item in seasons) Padding(padding: const EdgeInsets.only(right: 10), child: ChoiceChip(
                label: Text(item.number == 0 ? 'Specials' : 'S${item.number}'),
                selected: season?.number == item.number,
                onSelected: (_) => setState(() => selectedSeason = item.number),
              )),
            ])),
            const SizedBox(height: 20),
            Text('${progress.unseen.length} episodes to watch', style: Theme.of(context).textTheme.titleMedium),
            ...
            if (season == null) const Text('No seasons available.') else ...[
              ...
              for (final episode in [...season.episodes]..sort(episodeOrder)) ...[
                ListTile(contentPadding: const EdgeInsets.symmetric(vertical: 8),
                  leading: IconButton(
                    tooltip: watched.containsKey(episode.key) ? 'Mark as unwatched' : 'Mark as watched',
                    icon: Icon(watched.containsKey(episode.key) ? Icons.check_circle : Icons.radio_button_unchecked),
                    onPressed: saved && (episode.released(DateTime.now()) || watched.containsKey(episode.key))
                        ? () => perform(context, () => markEpisodeWithConfirmation(context, ref, episode, !watched.containsKey(episode.key))) : null,
                  ),
                  title: Text(episode.title),
                  subtitle: Text('${episode.code} · ${dateLabel(episode.airDate)}'),
                  trailing: const Icon(Icons.chevron_right),
                  onTap: () => openScreen(context, EpisodeScreen(title: title, episode: episode, fromSeries: true)),
                ),
    ...
    body: ref
        ...
        .when(
          ...
          data: (loaded) {
            ...
            if (season == null) {
              return const Center(
                child: Text('This season is no longer available in the catalog.'),
              );
            }
            final watched = snapshot?.watched ?? <String, DateTime>{};
              ...
              children: [
                ...
                if (season.episodes.isEmpty)
                  const Padding(
                    ...
                    child: Text('Season announced, with no episodes available yet.'),
                  ),
                for (final episode in [...season.episodes]..sort(episodeOrder))
                  ListTile(
...
class EpisodeScreen extends ConsumerWidget {
  ...
  const EpisodeScreen({super.key, required this.title, required this.episode, this.fromSeries = false});
  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final snapshot = ref.watch(libraryProvider).asData?.value;
    final current =
    ...
    final crew = current.crew
        ...
        .join(', ');
    return Scaffold(
      appBar: AppBar(automaticallyImplyLeading: false, leading: IconButton(tooltip: 'Close', icon: const Icon(Icons.close), onPressed: () => Navigator.pop(context))),
      body: ListView(primary: true,
        ...
        children: [
          ...
          const SizedBox(height: 24),
          if (current.still.isNotEmpty) Poster(current.still, width: double.infinity, height: 240),
          const SizedBox(height: 24),
